...
#ifndef MEETING_COPILOT_BUNDLE_ID
	#define MEETING_COPILOT_BUNDLE_ID "io.github.bb8ad8.meetron.audio"
#endif

#ifndef MEETING_COPILOT_BOX_UID
	#define MEETING_COPILOT_BOX_UID "io.github.bb8ad8.meetron.audio.box"
#endif

#ifndef MEETING_COPILOT_DEVICE_UID
	#define MEETING_COPILOT_DEVICE_UID "io.github.bb8ad8.meetron.audio.device"
#endif

#ifndef MEETING_COPILOT_MODEL_UID
	#define MEETING_COPILOT_MODEL_UID "io.github.bb8ad8.meetron.audio.model"
#endif

#ifndef MEETING_COPILOT_DEVICE_NAME
	#define MEETING_COPILOT_DEVICE_NAME "Meetron Audio"
#endif
...
//		- reads the final output mix from a fixed ring buffer
//	- a single output stream
//		- supports 2 channels of 32 bit float LPCM samples
//		- writes the final mix to a fixed ring buffer
//	- controls
//		- master input volume
		...
		case kObjectID_DataDestination_PlayThru_Master:
			switch(inAddress->mSelector)
			{
				...
				case kAudioSelectorControlPropertyItemName:
					...
					*((CFStringRef*)outData) = CFSTR(MEETING_COPILOT_DEVICE_NAME);
					*outDataSize = sizeof(CFStringRef);
					break;
...
static OSStatus	NullAudio_StartIO(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID)
{
	...
	else if(gDevice_IOIsRunning == 0)
	{
		...
		MCAudioRingBufferReset(&gDevice_RingBuffer);
	}
	else
	{
...
static OSStatus	NullAudio_DoIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, AudioObjectID inStreamObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo, void* ioMainBuffer, void* ioSecondaryBuffer)
{
	//	Copy the final output mix into a fixed, preallocated ring buffer and expose those frames on
	//	the input stream. The IO callback performs no allocation, blocking IO, or locking.
	...
	FailWithAction(inIOCycleInfo == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done, "NullAudio_DoIOOperation: no cycle info");
	...
	{
		const Float64 theSampleTime = inIOCycleInfo->mOutputTime.mSampleTime;
		if(theSampleTime >= 0)
		{
			MCAudioRingBufferWrite(
				&gDevice_RingBuffer,
				(UInt64)theSampleTime,
				inIOBufferFrameSize,
				theBuffer
			);
		}
	...
	{
		const Float64 theSampleTime = inIOCycleInfo->mInputTime.mSampleTime;
		if(theSampleTime < 0)
		{
